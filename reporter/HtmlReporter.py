...
class HtmlReporter:
    ...
    def add_scenario_result_as_table_formatted_data(self,
                                                    scenario_name,
                                                    partition_1_row: tuple,
                                                    partition_2_row: tuple,
                                                    status,
                                                    comment='',
                                                    unmatched_columns_index=None):
        ...
        str_partition_1_row = ''
        for val in partition_1_row:
            str_partition_1_row += f'<td>{"" if val is None else val}</td>\n'
        row_1 = f'\n<tr class="{class_name}">\n' \
                f'<td>{self.scenario_number}</td>\n' \
                f'<td>{scenario_name}</td>\n' \
                f'{str_partition_1_row}' \
                f'<td class="{class_name}">{status}</td>\n' \
                f'<td style="word-wrap: break-word">{comment}</td>\n' \
                f'</tr>\n'
        self.data = self.data + row_1

        str_partition_2_row = ''
        for i, val in enumerate(partition_2_row):
            if unmatched_columns_index and i in unmatched_columns_index:
                str_partition_2_row += f'<td class="mismatch">{"" if val is None else val}</td>\n'
            else:
                str_partition_2_row += f'<td>{"" if val is None else val}</td>\n'

        row_2 = f'\n<tr class="{class_name}">\n' \
                f'<td></td>\n' \
                f'<td></td>\n' \
                f'{str_partition_2_row}' \
                f'<td class="{class_name}"></td>\n' \
                f'<td style="word-wrap: break-word"></td>\n' \
                f'</tr>\n'
        self.data = self.data + row_2

        str_blank_row = ''
        for i in range(0, len(partition_1_row)):
            str_blank_row += '<td></td>'
